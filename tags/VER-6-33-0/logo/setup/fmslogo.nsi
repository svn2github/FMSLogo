...
SetupUser.Done:
  ...
  StrCmp $LANGUAGE ${LANG_FRENCH} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "L'installateur ou bien FMSLogo est en cours d'exécution.$\nL'installation ne peut continuer." 
     Abort
  StrCmp $LANGUAGE ${LANG_GREEK} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "Ï åãêáôáóôÜôçò Þ ç FMSLogo åêôåëåßôáé Þäç.$\nÁõôÞ ç åãêáôÜóôáóç äå ìðïñåß íá óõíå÷éóôåß."
     Abort
  StrCmp $LANGUAGE ${LANG_RUSSIAN} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "Èëè èíñòàëëÿòîð èëè FMSLogo â íàñòîÿùåå âðåìÿ çàïóùåíû.$\nÓñòàíîâêà íå ìîæåò áûòü ïðîäîëæåíà." 
     Abort
...
checkifinstalled:
  ...
  StrCmp $LANGUAGE ${LANG_FRENCH} 0 +2
    MessageBox MB_YESNO "La version existante de FMSLogo doit être désinstallée pour continuer.$\nVoulez-vous la désinstaller?$\n$\n(Choisir Non terminera l'installation)" IDYES uninstall IDNO abort
  StrCmp $LANGUAGE ${LANG_GREEK} 0 +2
    MessageBox MB_YESNO "Ôï õðÜñ÷ïí áíôßãñáöï ôçò FMSLogo ðñÝðåé íá áðåãêáôáóôáèåß ðñéí óõíå÷éóôåß áõôÞ ç åãêáôÜóôáóç.$\nÈÝëåôå íá ôï åãêáôáóôÞóåôå;$\n$\n(ÅðéëÝãïíôáò ¼÷é ç åãêáôÜóôáóç èá êëåßóåé.)" IDYES uninstall IDNO abort
  StrCmp $LANGUAGE ${LANG_RUSSIAN} 0 +2
    MessageBox MB_YESNO "Ñóùåñòâóþùàÿ êîïèÿ FMSLogo äîëæíà áûòü óäàëåíà, ÷òîáû ïðîäîëæèòü.$\nÂû õîòèòå óäàëèòü åå?$\n$\n(Âûáîð Íåò ïðåðâåò óñòàíîâêó)" IDYES uninstall IDNO abort
  StrCmp $LANGUAGE ${LANG_CROATIAN} 0 +2
    MessageBox MB_YESNO "Postojeæa se verzija programa FMSLogo mora deinstalirati ako želiš nastaviti.$\nŽeliš li je deinstalirati?$\n$\n(Ako odabereš $\"Ne$\", instalacija æe se prekinuti)" IDYES uninstall IDNO abort
  ; default to English
...
Function un.onInit
  ...
  StrCmp $LANGUAGE ${LANG_PORTUGUESE} 0 +3
     ...
     Abort
  StrCmp $LANGUAGE ${LANG_FRENCH} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "L'installateur ou bien FMSLogo est encours d'exécution.$\nLa désinstallation ne peut continuer." 
     Abort
  StrCmp $LANGUAGE ${LANG_GREEK} 0 +3
...
CheckIfInstallExists.ShowError:
  ...
  StrCmp $LANGUAGE ${LANG_PORTUGUESE} 0 +3
     ...
     Abort
  StrCmp $LANGUAGE ${LANG_FRENCH} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "FMSLogo a déjà été supprimé ou vous n'avez pas la permission de le désinstaller.$\nLa désinstallation ne peut continuer." 
     Abort
  StrCmp $LANGUAGE ${LANG_GREEK} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "Either FMSLogo has already been deleted or you do not have permission to uninstall it.$\nEither way, this uninstallation cannot continue." ; NOT_YET_LOCALIZED
     Abort
  StrCmp $LANGUAGE ${LANG_RUSSIAN} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "Èëè FMSLogo áûë óæå óäàëåí, èëè Âû íå èìååòå ïðàâ óäàëÿòü åå.$\nÂ ëþáîì ñëó÷àå, óäàëåíèå íå ìîæåò áûòü ïðîäîëæåíî." 
     Abort
  StrCmp $LANGUAGE ${LANG_CROATIAN} 0 +3
     MessageBox MB_OK|MB_ICONEXCLAMATION "Ili je FMSLogo veæ izbrisan ili nemaš dopuštenje da ga deinstaliraš.$\nU svakom sluèaju, ova se deinstalacija ne može nastaviti."
  ; default to English
